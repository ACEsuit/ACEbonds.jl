...
abstract type AbstractBondsIterator end

function _get_bond_env(iter::AbstractBondsIterator, i, j, rrij)
...
struct BondsIterator <: AbstractBondsIterator
   at
   nlist_bond
   nlist_env 
   env_filter 
...
* `env_filter` : `filter(X) == true` if particle `X` is to be included; `false` if to be discarded from the environment
"""
function bonds(at::Atoms, rcutbond, rcutenv, env_filter) 
   ...
   return BondsIterator(at, nlist_bond, nlist_env, env_filter)
end
...
struct FilteredBondsIterator <: AbstractBondsIterator
   at
   nlist_bond
   nlist_env 
   env_filter
